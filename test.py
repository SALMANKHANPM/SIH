...
class CustomJSONLoader(JSONLoader):
    def _get_text(self, sample):
        # Convert the dict to a string if it's a dict
        return json.dumps(sample) if isinstance(sample, dict) else super()._get_text(sample)
...
if __name__ == "__main__":
    ...
    # Create vector store
    vectorstore = create_vectorstore(splits, model_name="tinyllama", persist_directory='db')

    # Create RAG chain
    conversational_rag_chain = create_rag_chain(vectorstore, model_name="tinyllama")
